...
#!/usr/bin/env python3
"""
Robust FastAPI app with non-blocking background worker (lifespan-based).
Paste this as app.py and deploy. Logs errors clearly.
"""
...
TICKERS = [t for t in os.environ.get("TICKERS", "ADANIPOWER.NS").split(",") if t]
...
def _fetch_last_1m_candle(ticker: str):
    try:
        ...
        # make sure exceptions are visible in logs
        print(f"[fetch] yfinance error for {ticker}: {e}", flush=True)
        return None
...
# --- FastAPI instance (no blocking startup) ------------------------------
app = FastAPI(title="1min Open/Close Appender")
...
async def background_worker_loop(tickers: List[str]):
    ...
    try:
        while not _shutdown_event.is_set():
            ...
            for tk in tickers:
                try:
                    ...
                    print(f"[background] fetch exception {tk}: {e}", flush=True)
                    res = None
                if res is None:
                    continue
                ...
                try:
                    ...
                    if appended:
                        print(f"[background] {tk} appended {appended} row(s) at {ts_str}", flush=True)
                except Exception as e:
                    print(f"[background] write error for {tk}: {e}", flush=True)
        ...
        print("[background] stopped", flush=True)

# --- lifespan: start/stop background task cleanly --------------------------
...
async def lifespan(app: FastAPI):
    ...
    if ENABLE_BACKGROUND and TICKERS:
        _background_task = asyncio.create_task(background_worker_loop(TICKERS))
    try:
        yield
    finally:
        _shutdown_event.set()
...
# mount lifespan
app = FastAPI(title="1min Open/Close Appender", lifespan=lifespan)

# --- endpoints ------------------------------------------------------------
@app.get("/")
async def root():
    now = datetime.now().astimezone(IST).strftime("%Y-%m-%d %H:%M:%S %Z")
    return {"ok": True, "time": now, "background_enabled": ENABLE_BACKGROUND, "tickers": TICKERS}

@app.get("/run")
@app.post("/run")
async def run_once(ticker: str = Query(...), key: Optional[str] = Query(None)):
    if API_KEY:
        if key != API_KEY:
